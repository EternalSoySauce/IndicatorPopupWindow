...
class IndicatorPopupWindow(context: Context, private val indicatorParams: IndicatorParams) :
     ...
     */
    fun showAtAnchorView(view: View, relativeX: RelativeX = RelativeX.RIGHT, relativeY: RelativeY = RelativeY.BOTTOM, offsetX: Int = 0, offsetY: Int = 0, ignoreStatusBar: Boolean = false) {
        ...
        when (relativeX) {
            RelativeX.LEFT -> {
                while (true) {
                    arrowLayoutParams.leftMargin = (contentLayout.measuredWidth - indicatorParams.arrowWidth - arrowMargin)
                            .coerceIn(0, contentLayout.measuredWidth - indicatorParams.arrowWidth)
                    realX = viewLocation[0] + view.width / 2 - arrowLayoutParams.leftMargin - indicatorParams.arrowWidth / 2 - offsetX
                    if (contentLayout.layoutParams.width == 0) {
                        break
                    } else if (realX - indicatorParams.marginScreenHor < 0) {
                        contentLayout.layoutParams.width = (contentLayout.measuredWidth - (indicatorParams.marginScreenHor - realX)).coerceAtLeast(0)
                        rootLayout.measure(0, 0)
                    } else {
                        break
                    }
            ...
            RelativeX.MIDDLE -> {
                while (true) {
                    arrowLayoutParams.leftMargin = ((contentLayout.measuredWidth - indicatorParams.arrowWidth) / 2 + arrowMargin)
                            .coerceIn(0, contentLayout.measuredWidth - indicatorParams.arrowWidth)
                    realX = viewLocation[0] + view.width / 2 - arrowLayoutParams.leftMargin - indicatorParams.arrowWidth / 2 + offsetX
                    if (contentLayout.layoutParams.width == 0) {
                        break
                        ...
                        contentLayout.layoutParams.width = (contentLayout.measuredWidth - (realX + contentLayout.measuredWidth + indicatorParams.marginScreenHor - screenSize[0])).coerceAtLeast(0)
                        rootLayout.measure(0, 0)
                    } else {
                        break
                    }
                ...
                arrowLayoutParams.leftMargin = arrowMargin.coerceIn(0, contentLayout.measuredWidth - indicatorParams.arrowWidth)
                realX = viewLocation[0] + view.width / 2 - arrowLayoutParams.leftMargin - indicatorParams.arrowWidth / 2 + offsetX
        ...
        when (relativeY) {
            RelativeY.TOP -> {
                while (true) {
                    realY = viewLocation[1] - contentLayout.measuredHeight - indicatorParams.arrowHeight - offsetY
                    if (contentLayout.layoutParams.height == 0) {
                        break
                    } else if (realY - statusBarHeight - indicatorParams.marginScreenVer < 0) {
                        contentLayout.layoutParams.height = (contentLayout.measuredHeight - (indicatorParams.marginScreenVer + statusBarHeight - realY)).coerceAtLeast(0)
                        rootLayout.measure(0, 0)
                    } else {
                        break
                    }
                ...
                val bottomPos = realY + contentLayout.measuredHeight + indicatorParams.arrowHeight - statusBarHeight + indicatorParams.marginScreenVer
                if (bottomPos > screenSize[1]) {
                    contentLayout.layoutParams.height = (contentLayout.measuredHeight - (bottomPos - screenSize[1])).coerceAtLeast(0)
                }
            }
        }
    ...
    private fun createArrowView(context: Context, @TriangleDrawable.ArrowDirection arrowDirection: Int): View {
    ...
    private fun createContentLayout(context: Context): ViewGroup {
        ...
        contentLayout.layoutParams = FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT)
        contentLayout.background = drawable

        val textView: TextView
        if (indicatorParams.contentLayoutResId != 0 && indicatorParams.contentTextViewId != 0) {
            val contentView = LayoutInflater.from(context).inflate(indicatorParams.contentLayoutResId, null, false)
